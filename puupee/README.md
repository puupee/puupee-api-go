...
```golang
import doggy "github.com/jiejie-dev/puupee-api-go/doggy"
```
...
```golang
ctx := context.WithValue(context.Background(), doggy.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), doggy.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), doggy.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), doggy.ContextOperationServerVariables, map[string]map[string]string{
...
## Documentation For Models
 ...
 - [CreateUpdateAppUserScoreDto](docs/CreateUpdateAppUserScoreDto.md)
 - [CreateUpdatePuupeeDto](docs/CreateUpdatePuupeeDto.md)
